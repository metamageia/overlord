...
**Agent guidance:** Do **not** generate, modify, or inject `statblockID` or `bundleId` when creating or editing bundles programmatically. These IDs are produced by Overlord (via `idManagement.mjs`) or explicitly set by a human user; they should be omitted from JSON bundles by default unless the user requests otherwise.
...
```javascript
{
  ...
  // NOTE: `statblockID` and `bundleId` are *not* required when providing bundle JSONs
  // and should generally be omitted. These IDs are generated by Overlord or set by a user,
  // and **agents must not add or modify them** when producing or updating bundles.
  // If present, they must be correct content-hashes; otherwise leave them out.
...
- **Agent policy for IDs**: When authoring or modifying bundles, AI agents must **not** add or change `statblockID` or `bundleId`. Leave these fields out of generated JSON by default; let Overlord or the user generate or update them.
